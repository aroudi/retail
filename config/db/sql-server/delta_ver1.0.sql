...
ALTER TABLE PURCHASE_ORDER_HEADER ADD POH_PRJ_NAME VARCHAR(1000)
GO

ALTER TABLE PURCHASE_ORDER_HEADER ADD POH_PRJ_CODE VARCHAR(500)
GO

ALTER TABLE DELIVERY_NOTE_HEADER ADD DELN_SURCHARGE DECIMAL(31,6)
GO


-- UP TO THIS POINT APPLIED TO JOMON ON 19/01/2017
...
INSERT INTO CUSTOMER_GRADE(GRADE_CODE, GRADE_NAME, DESCRIPTION, RATE) VALUES ('Default', 'Default', 'COST + 30.00%','0.30')
GO

-- UP TO THIS POINT APPLIED TO JOMON ON 06/02/2017
...
ALTER TABLE TXN_HEADER ADD TXHD_PO_NR varchar(200)
GO

ALTER TABLE INVOICE ADD TXIV_PO_NR varchar(200)
GO

--  UP TO THIS POINT APPLIED TO JOMON ON 23/02/2017
ALTER TABLE TXN_HEADER ADD TXHD_EMAIL_TO VARCHAR(100)
GO

ALTER TABLE INVOICE ADD TXIV_EMAIL_TO VARCHAR(100)
GO

ALTER TABLE INVOICE ADD     POH_EMAIL_TO VARCHAR(100)
GO
--  UP TO THIS POINT APPLIED TO JOMON ON 06/03/2017
...
--  UP TO THIS POINT APPLIED TO JOMON ON 16/03/2017

ALTER TABLE SUPP_PROD_PRICE ADD COST_BEFORE_TAX DECIMAL(31,6)
GO

ALTER TABLE SUPP_PROD_PRICE ADD BULK_PRICE_BEFORE_TAX DECIMAL(31,6)
GO

ALTER TABLE SUPP_PROD_PRICE ADD TXLV_ID DECIMAL(31,0)
GO
ALTER TABLE SUPP_PROD_PRICE ADD CONSTRAINT SPRC_FK6 FOREIGN KEY (TXLV_ID) REFERENCES TAX_LEG_VARIANCE(TXLV_ID)
go

ALTER TABLE PURCHASE_ORDER_HEADER ADD POH_VALUE_TAX DECIMAL(31,6)
GO

ALTER TABLE PURCHASE_LINE ADD POL_VALUE_TAX DECIMAL(31,6)
GO

ALTER TABLE PURCHASE_LINE ADD POL_VALUE_GROSS DECIMAL(31,6)
GO

UPDATE TAX_LEG_VARIANCE SET TXLV_RATE = 0.12 WHERE TXLV_CODE = 'S12'
GO

UPDATE TAX_LEG_VARIANCE SET TXLV_RATE = 0.22 WHERE TXLV_CODE = 'S22'
GO

ALTER TABLE PURCHASE_ORDER_HEADER ADD COSTS_INCLUDE_TAX BIT DEFAULT 0
GO

ALTER TABLE DELIVERY_NOTE_LINE ADD DLNL_VALUE_TAX DECIMAL(31,6)
GO

ALTER TABLE DELIVERY_NOTE_LINE ADD DLNL_VALUE_GROSS DECIMAL(31,6)
GO

ALTER TABLE DELIVERY_NOTE_LINE ADD TXLV_ID DECIMAL(31,0)
GO
...
--  UP TO THIS POINT APPLIED TO JOMON ON 16/05/2017
...
alter table SUPP_PROD_PRICE
  add BULK_QTY2 DECIMAL(31,4)
go

alter table SUPP_PROD_PRICE
  add BULK_PRICE2 DECIMAL(31,6) --AFTER TAX
go
alter table SUPP_PROD_PRICE
  add BULK_QTY3 DECIMAL(31,4)
go

alter table SUPP_PROD_PRICE
  add BULK_PRICE3 DECIMAL(31,6) --AFTER TAX
go
alter table SUPP_PROD_PRICE
  add BULK_QTY4 DECIMAL(31,4)
go

alter table SUPP_PROD_PRICE
  add BULK_PRICE4 DECIMAL(31,6) --AFTER TAX
go
alter table SUPP_PROD_PRICE
  add BULK_QTY5 DECIMAL(31,4)
go
...
go
INSERT INTO ROLE_ACCESS(ROLE_ID, ACPT_ID) VALUES ( (select ROLE_ID from APP_ROLE WHERE ROLE_CODE = 'admin'), (select ACPT_ID FROM ACCESS_POINT WHERE ACPT_TOKEN = 'productPriceChangeBulk' ));
go

alter table PRODUCT
  add PROD_BARCODE VARCHAR(500)--AFTER TAX
go
--up to this point applied on JOMON on 03/07/17

CREATE TABLE PO_SO_LINK(
    POSO_ID DECIMAL(31,0)IDENTITY (1,1) NOT NULL,
...
-- define a new category code for po creation type which is created from Sale order
update CONFIG_CATEGORY set DISPLAY_NAME='FROM BOQ',  DESCRIPTION='FROM BOQ' where CATEGORY_CODE= 'POH_CREATION_TYPE_AUTO';
...
alter table TXN_DETAIL
  add TXDE_QTY_ORDERED DECIMAL(31,4) -- BACK ORDER QTY
go

alter table TXN_DETAIL
  add TXDE_QTY_RECEIVED DECIMAL(31,4) -- BACK ORDER QTY
go
...
alter table TXN_HEADER
   add TXHD_PRJ_CODE VARCHAR(500)
go

alter table DELIVERY_NOTE_HEADER
   add DELN_PRJ_CODE VARCHAR(500)
go
...
alter table INVOICE
   add TXIV_PRJ_CODE VARCHAR(500)
go

update SUPP_PROD_PRICE set SPRC_PREFER_BUY = 1
GO

ALTER TABLE DELIVERY_NOTE_HEADER drop DELIVERY_NOTE_HEADER_FK2
go

alter table STOCK_EVENT
   add     TXN_TYPE_RESERVED_FOR VARCHAR(200)
go

alter table STOCK_EVENT
   add     TXN_NR_RESERVED_FOR DECIMAL(31,0)
go

alter table STOCK_EVENT
   add     TXN_ITEM_RESERVED_FOR DECIMAL(31,0)
go
...
alter table TXN_DETAIL
  add TXDE_QTY_BACK_ORDER DECIMAL(31,4) -- BACK ORDER QTY
go

alter table STOCK_EVENT
   drop column     TXN_NR_RESERVED_FOR
go

alter table STOCK_EVENT
   add     TXN_NR_RESERVED_FOR VARCHAR(200)
go

alter table STOCK_EVENT
   add     TXN_ID_RESERVED_FOR DECIMAL(31,0)
go

CREATE TABLE STOCK_RESERVE(
    STCK_RSV_ID DECIMAL(31,0) IDENTITY (1,1) NOT NULL,
    STCK_ID DECIMAL(31,0) NOT NULL,
    ORGU_ID DECIMAL(31,0),
    STORE_ID DECIMAL(31,0),
    SUPPLIER_ID DECIMAL(31,0) NULL,
    ...
    TXN_HEADER_ID DECIMAL(31,0),
    TXN_LINE_ID DECIMAL(31,0),
    TXN_NUMBER VARCHAR(200),
    QTY_RESERVED DECIMAL(31,4)
);

ALTER TABLE STOCK DROP CONSTRAINT STOCK_FK5
GO
ALTER TABLE STOCK_EVENT DROP CONSTRAINT STOCK_EVENT_FK12
GO
-- up to this point deployed on jomon on 20/08/17 (ver 2.0)

ALTER TABLE PRODUCT DROP CONSTRAINT PROD_UK1
GO
...
alter table CONTACT
   	add E_MAIL2 varchar(100) NULL
go

alter table Customer
   add  CONTACT_ID DECIMAL(31,0) NULL
go
...
-- up to this point deployed on jomon on 08/02/18 (ver 2.6)

alter table TXN_HEADER
   add TXHD_COMMENT VARCHAR(1024)
go
...
CREATE TABLE DEPARTMENT  (
     DEPT_ID DECIMAL(31,0) IDENTITY (1,1) NOT NULL,
...
CREATE TABLE CATEGORY_HEADING  (
     CATH_ID DECIMAL(31,0) IDENTITY (1,1) NOT NULL,
...
CREATE TABLE CATEGORY_VALUE  (
     CATV_ID DECIMAL(31,0) IDENTITY (1,1) NOT NULL,
     CATH_ID DECIMAL(31,0) NOT NULL,
     ACTIVE BIT DEFAULT 1,
...
CREATE TABLE DEPT_CATEGORY  (
     DEPT_CAT_ID DECIMAL(31,0) IDENTITY (1,1) NOT NULL,
     DEPT_ID DECIMAL(31,0) NOT NULL,
...
CREATE TABLE DEPT_CATEGORY_VALUE  (
     DEPT_CAT_VAL_ID DECIMAL(31,0) IDENTITY (1,1) NOT NULL,
     DEPT_CAT_ID DECIMAL(31,0) NOT NULL,
     DEPT_ID DECIMAL(31,0) NOT NULL,
     CATH_ID DECIMAL(31,0) NOT NULL,
     CATV_ID DECIMAL(31,0) NOT NULL
   )
go

CREATE TABLE PROD_DEPT_CAT  (
     ID DECIMAL(31,0) IDENTITY (1,1) NOT NULL,
     ORGU_ID DECIMAL(31,0) NOT NULL,
     PROD_ID DECIMAL(31,0) NOT NULL,
     DEPT_ID DECIMAL(31,0) NOT NULL,    -- ONE DEFAULT DEPARTMENT SHOULD BE DEFINED
     CAT_ID DECIMAL(31,0),
     LEVEL DECIMAL (5,0),
     CAT_VAL_ID DECIMAL(31,0)
   )
go
...
-- up to this point updated to JOMON on 20/10/2018

ALTER TABLE SESSION_MEDIA ADD TXMD_ID DECIMAL(31,0)
GO
...
ALTER TABLE STORE ADD CASH_SESSION_TYPE DECIMAL(31,0)
GO

UPDATE STORE SET CASH_SESSION_TYPE = (SELECT CATEGORY_ID FROM CONFIG_CATEGORY WHERE CATEGORY_CODE = 'SESSION_PER_COMPANY')
...
alter table stock_event drop constraint STOCK_EVENT_FK1
go

alter table stock_event drop constraint STOCK_EVENT_FK8
go
alter table stock_event drop constraint STOCK_EVENT_FK9
go
alter table stock_event drop constraint STOCK_EVENT_FK10
go
...
UPDATE CONFIG_CATEGORY SET CATEGORY_CODE = 'PROD_STATUS_FINALISED' , DISPLAY_NAME = 'FINALISED', DESCRIPTION = 'FINALISED' WHERE CATEGORY_CODE = 'POS'
GO
UPDATE CONFIG_CATEGORY SET CATEGORY_CODE = 'PROD_STATUS_IN_REVIEW' , DISPLAY_NAME = 'IN REVIEW', DESCRIPTION = 'IN REVIEW' WHERE CATEGORY_CODE = 'IMPORTED'
GO
...
GO
-- BOQ TEMPORARY RECORD

ALTER TABLE SUPPLIER
  ADD VERIFIED BIT DEFAULT 1 NOT NULL
GO
...
update CONFIG_CATEGORY set DISPLAY_NAME = 'PENDING', DESCRIPTION = 'PENDING', COLOR = 'red' WHERE CATEGORY_CODE = 'BOQ_STATUS_NEW'
go
...
alter table TXN_MEDIA
	add    TXMD_REFUNDED BIT DEFAULT 0 NOT NULL
go
...
update CONFIG_CATEGORY set COLOR = 'blue' where CATEGORY_CODE = 'TXN_TYPE_SALE'
go
update CONFIG_CATEGORY set COLOR = 'red' where CATEGORY_CODE = 'TXN_TYPE_REFUND'
go
update CONFIG_CATEGORY set COLOR = 'yellow' where CATEGORY_CODE = 'TXN_TYPE_QUOTE'
go
update CONFIG_CATEGORY set COLOR = 'green' where CATEGORY_CODE = 'TXN_TYPE_INVOICE'
go
update CONFIG_CATEGORY set COLOR = 'purple' where CATEGORY_CODE = 'TXN_TYPE_BOQ'
go